...
piA=[];
 ...
 %gth_1= rgb2gray(imread('ct_liver.png'));
...
for i = 2:r+1
    for j = 2:k+1
        ...
        d1 = 2 - (1-A+t1).*exp(A-t1)-(1-t1+A).*exp(t1-A)+2-(1+pit1-piA).*exp(piA-pit1)-(1-pit1+piA).*exp(pit1-piA);
        min1 =min(min(d1));
        d2 = 2 - (1-A+t2).*exp(A-t2)-(1-t2+A).*exp(t2-A)+2-(1+pit2-piA).*exp(piA-pit2)-(1-pit2+piA).*exp(pit2-piA);
        min2 =min(min(d2));
        d3 = 2 - (1-A+t3).*exp(A-t3)-(1-t3+A).*exp(t3-A)+2-(1+pit3-piA).*exp(piA-pit3)-(1-pit3+piA).*exp(pit3-piA);
        min3 =min(min(d3));
        d4 = 2 - (1-A+t4).*exp(A-t4)-(1-t4+A).*exp(t4-A)+2-(1+pit4-piA).*exp(piA-pit4)-(1-pit4+piA).*exp(pit4-piA);
        min4 =min(min(d4));
        d5 = 2 - (1-A+t5).*exp(A-t5)-(1-t5+A).*exp(t5-A)+2-(1+pit5-piA).*exp(piA-pit5)-(1-pit5+piA).*exp(pit5-piA);
        min5 =min(min(d5));
        d6 = 2 - (1-A+t6).*exp(A-t6)-(1-t6+A).*exp(t6-A)+2-(1+pit6-piA).*exp(piA-pit6)-(1-pit6+piA).*exp(pit6-piA);
        min6 =min(min(d6));
        d7 = 2 - (1-A+t7).*exp(A-t7)-(1-t7+A).*exp(t7-A)+2-(1+pit7-piA).*exp(piA-pit7)-(1-pit7+piA).*exp(pit7-piA);
        min7 =min(min(d7));
        d8 = 2 - (1-A+t8).*exp(A-t8)-(1-t8+A).*exp(t8-A)+2-(1+pit8-piA).*exp(piA-pit8)-(1-pit8+piA).*exp(pit8-piA);
        min8 =min(min(d8));
        d9 = 2 - (1-A+t9).*exp(A-t9)-(1-t9+A).*exp(t9-A)+2-(1+pit9-piA).*exp(piA-pit9)-(1-pit9+piA).*exp(pit9-piA);
        min9 =min(min(d9));
        d10 = 2 - (1-A+t10).*exp(A-t10)-(1-t10+A).*exp(t10-A)+2-(1+pit10-piA).*exp(piA-pit10)-(1-pit10+piA).*exp(pit10-piA);
        min10 =min(min(d10));
        d11 = 2 - (1-A+t11).*exp(A-t11)-(1-t11+A).*exp(t11-A)+2-(1+pit11-piA).*exp(piA-pit11)-(1-pit11+piA).*exp(pit11-piA);
        min11 =min(min(d11));
        d12 = 2 - (1-A+t12).*exp(A-t12)-(1-t12+A).*exp(t12-A)+2-(1+pit12-piA).*exp(piA-pit12)-(1-pit12+piA).*exp(pit12-piA);
        min12 =min(min(d12));
        d13 = 2 - (1-A+t13).*exp(A-t13)-(1-t13+A).*exp(t13-A)+2-(1+pit13-piA).*exp(piA-pit13)-(1-pit13+piA).*exp(pit13-piA);
        min13 =min(min(d13));
        d14 = 2 - (1-A+t14).*exp(A-t14)-(1-t14+A).*exp(t14-A)+2-(1+pit14-piA).*exp(piA-pit14)-(1-pit14+piA).*exp(pit14-piA);
        min14 =min(min(d14));
        d15 = 2 - (1-A+t15).*exp(A-t15)-(1-t15+A).*exp(t15-A)+2-(1+pit15-piA).*exp(piA-pit15)-(1-pit15+piA).*exp(pit15-piA);
        min15 =min(min(d15));
        d16 = 2 - (1-A+t16).*exp(A-t16)-(1-t16+A).*exp(t16-A)+2-(1+pit16-piA).*exp(piA-pit16)-(1-pit16+piA).*exp(pit16-piA);
        min16 =min(min(d16));
